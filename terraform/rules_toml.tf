locals {
  rule_files = fileset(path.module, "../detection-rules/custom-rules/*.toml")
...
resource "elasticstack_kibana_security_detection_rule" "toml_rules_dev" {
  for_each = var.environment == "dev" ? local.normalized_rules : {}
...
resource "elasticstack_kibana_security_detection_rule" "toml_rules_prod" {
  for_each = var.environment == "prod" ? local.normalized_rules : {}
