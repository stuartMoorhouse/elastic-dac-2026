...
resource "elasticstack_kibana_exception_list" "svc_account_login_dev" {
...
resource "elasticstack_kibana_exception_list" "svc_account_login_prod" {
...
resource "elasticstack_kibana_exception_list_item" "svc_sqlbackup_dev" {
  count    = var.environment == "dev" ? 1 : 0
  provider = elasticstack.dev

  name           = "Exclude svc_sqlbackup"
  item_id        = "svc-sqlbackup-exclusion"
  list_id        = elasticstack_kibana_exception_list.svc_account_login_dev[0].list_id
  namespace_type = "single"
...
resource "elasticstack_kibana_exception_list_item" "svc_sqlbackup_prod" {
  count    = var.environment == "prod" ? 1 : 0
  provider = elasticstack.prod

  name           = "Exclude svc_sqlbackup"
  item_id        = "svc-sqlbackup-exclusion"
  list_id        = elasticstack_kibana_exception_list.svc_account_login_prod[0].list_id
  namespace_type = "single"
  description    = "SQL backup service account — interactive logins are expected during maintenance windows"
  type           = "simple"
...
resource "elasticstack_kibana_security_detection_rule" "svc_account_login_dev" {
  count    = var.environment == "dev" ? 1 : 0
  provider = elasticstack.dev
  ...
  exceptions_list = [
    {
      id             = elasticstack_kibana_exception_list.svc_account_login_dev[0].id
      list_id        = elasticstack_kibana_exception_list.svc_account_login_dev[0].list_id
      namespace_type = "single"
      type           = "detection"
    }
...
resource "elasticstack_kibana_security_detection_rule" "svc_account_login_prod" {
  count    = var.environment == "prod" ? 1 : 0
  provider = elasticstack.prod
  ...
  exceptions_list = [
    {
      id             = elasticstack_kibana_exception_list.svc_account_login_prod[0].id
      list_id        = elasticstack_kibana_exception_list.svc_account_login_prod[0].list_id
      namespace_type = "single"
      type           = "detection"
    }
